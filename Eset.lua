...
chats = {}
day = 86400
bot_id = 409300700 -- [[محل قرار گیری آیدی اکانت ربات]]
sudo_users = {189308877} -- [[محل قرار گیری آیدی سودو ها]] + [[مدیر ربات]]
bot_owner = {189308877} -- [[ محل قرار گیری آیدی مدیر اصلی ربات ]]
-----------------------------------------------------------------------------------------------
---------------
-- Start Functions --
---------------
        ...
        elseif msg_type == 'MSG:Text' then
          ...
          if database:get("bot:group:link"..msg.chat_id_) == 'waiting' and is_mod(msg.sender_user_id_, msg.chat_id_) then
            if text:match("(https://telegram.me/joinchat/%S+)") or text:match("(https://t.me/joinchat/%S+)") then
              ...
              send(msg.chat_id_, msg.id_, 1, ' لینک شما ثبت شد', 1, 'md')
            end
          end
          ...
          if database:get('bot:cmds'..msg.chat_id_) and not is_mod(msg.sender_user_id_, msg.chat_id_) then
            ...
            if text:match("^[Pp]ing$") then
              send(msg.chat_id_, msg.id_, 1, 'انلاینم :-)', 1, 'md')
            end
            -----------------------------------------------------------------------------------------------
            ...
            --[[if text:match("^[Ss]how edit$") and is_mod(msg.sender_user_id_, msg.chat_id_) then
              ...
              database:set('editmsg'..msg.chat_id_,'show')
              end]]
              -----------------------------------------------------------------------------------------------
              ...
              if text:match("^[Ll]ock (.*)$") and is_mod(msg.sender_user_id_, msg.chat_id_) then
                ...
                if mutept[2] == "mention" then
                  if not database:get('mention:lock'..msg.chat_id_) then
                    send(msg.chat_id_, msg.id_, 1, '> قفل اسم اشاره #فعال شد ! ', 1, 'md')
                    database:set('mention:lock'..msg.chat_id_,true)
                  else
              ...
              if text:match("^[Uu]nlock (.*)$") and is_mod(msg.sender_user_id_, msg.chat_id_) then
                ...
                if unmutept[2] == "mention" then
                  if database:get('mention:lock'..msg.chat_id_) then
                    ...
                    database:del('mention:lock'..msg.chat_id_)
                  else
              ...
              if text:match("^[Ss]hare$") and is_sudo(msg) then
                sendContact(msg.chat_id_, msg.id_, 0, 1, nil, 989015996389, 'Unes', 'Bigbang', 145064636)
              end
              -----------------------------------------------------------------------------------------------
...
Lock:[strict.spam.links.webpage.tag.hashtag.fwd.bots.edit.markdown.mention.pin.inline.farsi.english.tgservice.flood]
🔒قفل های اصلی:‌

Lock:[text.photo.video.gif.music.voice.file.sticker.contact.location]
 🔒قفل های رسانه:

Setspam [ 40 - به بالا ]
🔧مثلا شما میخواهید پیام های طولانی تر از 80 حرف پاک بشوند باید از دستور Setspam 80 
استفاده نمایید !
...
⚠️برای 📎 کردن یک پیام توسط ربات میتوانید پیام مورد نظر را ریپلای کنید سپس دستور pin را ارسال کنید
برای خارج کردن پیام از حالت 📎 میتوانید از دستور unpin استفاده کنید
برای مجدد 📎 کردن پیام سابق میتوانید از دستور repin استفاده نمایید

Setlink
✏️برایه تنظیم لینک گروه و پس از ثبت شدن از دستور
Link
استفاده نمایید!
...
lock gtime [1]
⏲برای فعال سازی به صورت زمان دار میتوانید به شکل بالا عمل کنید!

Lock cmd
⚠️بعضی از دستورات همگانی هستند ، یعنی به کاربر عادی هم پاسخ داده میشود ، اگر این حالت فعال باشد به آنها پاسخ نخواهد داد !
Del [100 به پایین]
♻️پاک کردن پیام های سوپرگروه!
 
Un[قفل مورد نظر]
🔓برای باز کردن قفل از مثال بالایی استفاده کنید!

بزودی راهنما کامل میشه :-)
BιɢBαɴɢ™*
]]
                                      send(msg.chat_id_, msg.id_, 1, text, 1, 'md')
   end
